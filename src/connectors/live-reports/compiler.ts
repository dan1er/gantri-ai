...
6. MUST NOT include a time period in \`title\` OR \`description\` of a parametric report (one whose steps use \`\$REPORT_RANGE\`). The viewer can change the window via the date picker, so any phrase claiming a fixed window — "Last 30 days", "Rolling 7-day snapshot", "Monthly performance", "Quarterly view", "for Q1 2026", "current month-to-date" — becomes a lie the moment they change the range. Describe what the report SHOWS (metrics, dimensions), not WHEN. Examples: title "Sales by Channel" — yes; "Sales by Channel — Last 7 Days" — no. description "Per-campaign Klaviyo email performance: opens, clicks, revenue, unsubs." — yes; "Rolling 30-day snapshot of Klaviyo email campaign performance." — no.
...
8. MUST NOT use internal system names ("Porter", "Transactions table", "rollup", "tool_result_cache") in \`title\`, \`description\`, table column labels, or any user-visible string. Most viewers are non-technical and don't recognize these. Translate: "Porter" → "Gantri orders" / "Gantri's order system"; "porter_order_id" column → "Order ID"; "rollup source" → omit (just describe what's shown). Real product names ("Northbeam", "Klaviyo", "Impact", "GA4") are fine — they're branded surfaces users recognize.
...
5. \`\$REPORT_RANGE\` token: every step arg that takes a date range — and only those — MUST be set to the literal string \`'\$REPORT_RANGE'\`. The runtime substitutes it with the picker value on every render. **Default to \`\$REPORT_RANGE\` for ANY report where a viewer might want to look at a different period.** This includes the user requesting "add date filtering", "make the range configurable", "default to last 30 days but let me change it", or any single-window report (sales by channel, late orders, daily reconciliation). \`\$REPORT_RANGE\` is the right answer ~95% of the time.
6. \`\$DATE:<base>[±Nd]\` macros: use these ONLY when the report locks in MULTIPLE distinct windows the viewer cannot change without breaking the report's meaning — e.g. "WTD this week vs WTD last week vs WTD 2 weeks ago" (3 simultaneous windows), "today vs same DOW last week" (paired comparison). NEVER use \`\$DATE:\` macros on a single-window report; that's what \`\$REPORT_RANGE\` is for. Reports that use \`\$DATE:\` macros are non-parametric (the picker is hidden because viewer interaction wouldn't be meaningful). Allowed bases: \`today\`, \`yesterday\`, \`this_monday\`, \`last_monday\`, \`monday_2w_ago\`, \`last_sunday\`, \`sunday_2w_ago\`. Offset is \`±Nd\` (days).
   DECISION RULE: if the user said "filter", "picker", "configurable", "change the period", "default to X", or didn't specify periods → \`\$REPORT_RANGE\`. If the user explicitly named ≥2 distinct windows that compare to each other → \`\$DATE:\` macros.
