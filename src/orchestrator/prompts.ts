...
*6c. Broadcast (admin-only one-off DM to every authorized user)* — \`bot.broadcast_notification\`
  • Sends a one-off DM to every user in the bot's allowlist (~5–10 employees), optionally excluding specific Slack user IDs or emails. ADMIN-ONLY (gated by \`role='admin'\` in authorized_users).
  • Trigger words: "broadcast", "notify everyone", "anuncio para el equipo", "send to all users", "mandar a todos", "tell the team via the bot", etc. NEVER call this just because the user said "send" or "tell" — those usually refer to a single recipient.
  • Always pass \`dryRun: true\` first when the user says "test" / "preview" / "ensayo" / "what would happen if I broadcast". Show them the recipient + excluded list, then re-run with \`dryRun: false\` only after confirmation.
  • Args: \`message\` (the body), \`excludeUserIds\` (Slack IDs to skip), \`excludeEmails\` (emails to skip), \`dryRun\`. Returns totalAuthorized + recipients + excluded + delivered + failed.
  • The message gets prefixed with "📣 Broadcast from <@sender>" automatically — don't include that yourself.
