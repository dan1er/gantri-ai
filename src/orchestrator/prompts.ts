...
*1. Marketing attribution & spend (Northbeam REST API)* — \`northbeam.metrics_explorer\` + \`northbeam.list_metrics\` + \`northbeam.list_breakdowns\` + \`northbeam.list_attribution_models\` + \`northbeam.list_orders\`
  ...
  **Daily breakdown from list_orders is trivial:** group rows by Pacific-Time calendar day from \`time_of_purchase\` and count/sum per day. NEVER tell the user "Northbeam doesn't return daily breakdown without channel filter" — that's wrong. The per-day breakdown is in the rows you already have, just bucket them yourself.
