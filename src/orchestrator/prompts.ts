...
*1b. Site behavior & realtime (GA4)* — \`ga4.run_report\` + \`ga4.realtime\`
  ...
  **For "list orders from Northbeam" / "give me the orders we have in NB for X month" / "how many orders did we have in NB yesterday/last week/in February" / "daily NB orders for the past 2 weeks" / "compare daily NB vs Grafana"** → call \`northbeam.list_orders\` with a \`dateRange\`. **PREFER \`list_orders\` over \`metrics_explorer\` for any "count of orders" or "daily order breakdown" question** — list_orders is a direct GET (sub-second, no async export polling) and returns per-order rows with \`time_of_purchase\` you can bucket by PT day client-side to get daily counts/revenue. metrics_explorer goes through the async export pipeline (POST → poll → CSV) which can take 5-60s and is overkill for a count.
  ...
  **Comparisons NB ↔ Grafana/Porter: ALWAYS use \`gantri.compare_orders_nb_vs_porter\`.** That tool runs a SINGLE canonical comparison (Porter side filtered to type=Order, status NOT IN Unpaid/Cancelled; NB side from /v2/orders, both PT-bucketed, both summing the same per-row revenue). Pass the rows it returns straight into a CSV via reports.attach_file — DO NOT re-aggregate, re-filter, or re-sum anywhere along the way. The LLM has been observed to drift on this composition (forgetting type=Order, mixing formulas, hand-summing revenue wrong) — using the dedicated tool eliminates that. Set \`excludeToday: true\` when the user says "exclude today" (or implies current-day NB lag is noise). Settled days will match exactly; only the current day shows NB ingestion lag.
...
*1b. Site behavior & realtime (Google Analytics 4 — GA4 Data API)* — \`ga4.run_report\`, \`ga4.realtime\`, \`ga4.list_events\`
  ...
  • **Page completion playbook — "qué páginas ven completas / scroll depth / how much of the page users see":**
    The GA4 default \`scroll\` event (Enhanced Measurement) fires once per session when the user reaches **90% of page depth**. If the property has it (verify via \`ga4.list_events\`), the ratio \`scroll_event_count / page_view_event_count\` per URL is the % of visits that scroll to the bottom — the right answer for these questions. Query (after confirming both events exist): \`ga4.run_report({dateRange:'last_30_days', dimensions:['pagePath','eventName'], metrics:['eventCount'], dimensionFilter:{filter:{fieldName:'eventName', inListFilter:{values:['page_view','scroll']}}}, limit:5000})\`. The filter cuts the response to ~1,500 rows (only the two events of interest, two rows per URL). Bucket per \`pagePath\`, divide \`scroll/page_view\`, rank. **Filter client-side to pages with at least 500 page_views** to avoid noise. **Caveat:** on home (/), /sign-up, /checkout the ratio can exceed 100% because scroll fires repeatedly there — flag those as "scroll listener fires multiple times" and exclude from the ranking.
  • **Example queries:**
