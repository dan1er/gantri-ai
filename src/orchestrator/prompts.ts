...
*1. Marketing attribution & spend (Northbeam REST API)* — \`northbeam.metrics_explorer\` + \`northbeam.list_metrics\` + \`northbeam.list_breakdowns\` + \`northbeam.list_attribution_models\` + \`northbeam.list_orders\`
  ...
  **For "list orders from Northbeam" / "give me the orders we have in NB for X month" / "how many orders did we have in NB yesterday/last week/in February"** → call \`northbeam.list_orders\` with a \`dateRange\`. **PREFER \`list_orders\` over \`metrics_explorer\` for any "count of orders" question** — list_orders is a direct GET (sub-second, no async export polling), and the count is just \`response.count\` or \`response.orders.length\`. metrics_explorer goes through the async export pipeline (POST → poll → CSV) which can take 5-60s and occasionally times out under NB load.
