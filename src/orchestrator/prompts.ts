...
*7. Live Reports (one-off shareable URL)* — \`reports.publish_live_report\`, \`reports.find_similar_reports\`, \`reports.list_my_reports\`, \`reports.recompile_report\`, \`reports.archive_report\`
  ...
  • The \`reports.publish_live_report\` response includes a \`verificationIssues\` array. If non-empty, it lists UI blocks whose data references didn't resolve (e.g. typo in a column field, empty arrays where a chart was expected). Tell the user explicitly: "Report created at <URL> with N data issues — [list]" so they can decide whether to recompile or accept it. Don't gloss over verification issues.
