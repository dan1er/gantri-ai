...
function makeStub(opts: {
  ...
  return {
    ...
    metricAggregateByName: vi.fn(async () => ({ dates: [], counts: [] })),
...
describe('klaviyo.consented_signups', () => {
  it('returns monthly counts from metric-aggregates', async () => {
    const client = {
      // mock the other methods that other tools call (just so the connector instantiates)
      listMetrics: vi.fn(),
      listSegments: vi.fn(),
      listCampaigns: vi.fn(),
      listFlows: vi.fn(),
      findMetricIdByName: vi.fn(),
      campaignValuesReport: vi.fn(),
      flowValuesReport: vi.fn(),
      segmentValuesReport: vi.fn(),
      metricAggregateByName: vi.fn().mockResolvedValue({
        dates: ['2026-01-01T08:00:00+00:00', '2026-02-01T08:00:00+00:00', '2026-03-01T08:00:00+00:00'],
        counts: [929, 504, 475],
      }),
    } as any;
    ...
    expect(out.period).toEqual({ startDate: '2026-01-01', endDate: '2026-03-31' });
  });

  it('passes interval=day for granularity=daily', async () => {
    const client = {
      listMetrics: vi.fn(), listSegments: vi.fn(), listCampaigns: vi.fn(), listFlows: vi.fn(),
  ...
  it('passes interval=week for granularity=weekly', async () => {
    const client = {
      listMetrics: vi.fn(), listSegments: vi.fn(), listCampaigns: vi.fn(), listFlows: vi.fn(),
      findMetricIdByName: vi.fn(), campaignValuesReport: vi.fn(), flowValuesReport: vi.fn(), segmentValuesReport: vi.fn(),
  ...
  it('returns empty rows when metric returns no data', async () => {
    const client = {
      listMetrics: vi.fn(), listSegments: vi.fn(), listCampaigns: vi.fn(), listFlows: vi.fn(),
      findMetricIdByName: vi.fn(), campaignValuesReport: vi.fn(), flowValuesReport: vi.fn(), segmentValuesReport: vi.fn(),
      metricAggregateByName: vi.fn().mockResolvedValue({ dates: [], counts: [] }),
